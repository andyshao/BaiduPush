...
namespace BaiduPushWebService
{
    public class BaiduPush
    {
        ...
        public BaiduPush(string httpMehtod)
        {
            this.httpMehtod = httpMehtod;
            this.url = SysFunction.GetConfigValue("BaiduPushUrl");
            this.secret_key = SysFunction.GetConfigValue("SecretKey");
        }

        public string PushMessage(PushOptions opts)
        {
            return Submit<PushOptions>(opts);
        }
        ...
        private string Submit<T>(T opts)
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();

            //将键值对按照key的升级排列
            var props = typeof(T).GetProperties().OrderBy(p => p.Name);
            ...
            try
            {
                ...
                byte[] responseData = webClient.UploadData(this.url, this.httpMehtod, data);//得到返回字符流  
